    ...
    public void generateReportBody(final Map<Deck, List<CardInList>> shopBuyList,
        ...
        shopBuyListByVendor.forEach((vendor, cardsInList) -> {
            ...
            cardsInList
                ...
                .forEach(entry -> {
                    body.append("  {").append(entry.getKey().toString()).append("}").append("\n");
                    entry.getValue().forEach((deck, cards) -> {
                        body.append("  ").append(deck.toString()).append("\n");
                        cards.forEach(card -> body.append("    ").append(card.toString()).append("\n"));
                    });
                });
